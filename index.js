...
const bot = new ViberBot({
    ...
    avatar: "https://i.imgur.com/6guPkWz.jpg" // It is recommended to be 720x720, and no more than 100kb.
});
        ...
        if (!isNaN(userInput)) {
            if (message.text) {
                ...
                if (trackingData[0].includes('MPT/') || trackingData[0].includes('Telenor/') || trackingData[0].includes('Ooredoo/') || trackingData[0].includes('MyTel/')) {
                    ...
                    bot.sendMessage(response.userProfile, [new TextMessage(`${name}-${denomination} Ks ${userInput} unit price is ${price} Ks,you want to calculate this price into retail price?`),
                        new KeyboardMessage({
